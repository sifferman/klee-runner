...
I measured 69.95\% overall line coverage (paper: 84.5\%) and
...
by the paper, achieving 63\% line and 70\% branch coverage in 60 minutes.
...
\Q{3}{Original setup} Each of 89 Coreutils 6.10 utilities is
...
10--24 KLEE instances concurrently to fit a class budget (each
...
measure 20.5\%, a near-exact match.
...
budget. \emph{(2) Measurement-script gaps}: the SHA-family
...
drop out of the aggregate. \emph{(3) Paper's per-tool tuning}:
...
replays a strict superset of Base's tests. Investigation showed
one \texttt{--max-fail 1} test crashes \texttt{mv} via
...
every digit, and \texttt{strtod} is highly branchy. The methodology
...
\Q{7}{Does the hypothesis still hold} Yes, convincingly. My
...
is the hypothesis. The shortfall is explained by factors
orthogonal to the algorithm (parallelism contention, measurement
artifacts), not by a breakdown of the technique. The
